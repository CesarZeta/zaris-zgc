...
- [x] Deploy (2026-07-04): backend en **Vercel serverless región gru1/São Paulo** (`https://zaris-zgc-api.vercel.app` — Railway se descartó: no tiene región SP; proyecto `zaris-zgc-api`, cuenta Vercel de César, env vars en el proyecto) + frontend en **GitHub Pages** (`https://cesarzeta.github.io/zaris-zgc/`, workflow `deploy-pages.yml`, variable de repo `API_URL`). Login E2E online verificado. ~~Pendiente menor: purga de commits huérfanos~~ → **HECHA 2026-07-12**: César borró el repo, Claude lo recreó (misma config: description/homepage/`API_URL`/Pages workflow/0 secrets), re-push de la historia limpia, Pages success y Vercel git reconectado (hizo falta `git disconnect` + `connect`: el webhook quedó atado al repo ID viejo y el primer push no deployó). El commit huérfano `b5722ba` dejó de existir.
...
- Regla de producto recordada por César en esta sesión: **una licencia POS = un
  ...
  decide sola.
- ~~Diferidos hallados en el click-through~~ → **RESUELTOS 2026-07-12 (misma
  fecha, pedido de César)**: (1) el multiplicador `3*texto` ya NO se pierde con
  resultados múltiples — `multi` queda vivo en el estado hasta que lo consume
  el camino directo, el modal de resultados o el de variantes (Esc lo limpia);
  (2) el POS resto registra **F8 = cierre de caja** como el mostrador (solo con
  la grilla a la vista, sin comanda ni modales abiertos).
- **Reserva de diseño (César, tras su recorrido 2026-07-12)**: el espacio libre
  abajo a la derecha del panel (bajo COBRAR) queda VACÍO a propósito — destino
