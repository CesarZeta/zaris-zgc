      ...
      <div className="modal modal-ancho" onClick={(ev) => ev.stopPropagation()}>
        ...
        <div className="fila">
          <div className="field" style={{ flex: 2 }}>
            <label>Cliente *</label>
            <BuscadorClienteMini elegido={cliente} onElegir={setCliente} />
          </div>
          <div className="field">
...
export default function CobranzasTab({ puntosVenta }: { puntosVenta: PuntoVenta[] }) {
  ...
  const cargar = useCallback(async () => {
    setCargando(true);
    try {
      const { data } = await apiGet<Recibo[]>("/cobranzas/recibos?limit=100");
      setRecibos(data);
    } catch (err) {
  ...
  async function anular(r: Recibo) {
    if (!window.confirm(`¿Anular el recibo ${r.numero_formateado}? Se revierten sus imputaciones.`))
      return;
    try {
    ...
    <>
      <div className="toolbar">
        <span className="page-sub" style={{ margin: 0 }}>
          {cargando ? "Cargando…" : `${recibos.length} recibos`}
        </span>
        <div style={{ flex: 1 }} />
      ...
      <div className="tabla-card">
          ...
          <tbody>
            {recibos.map((r) => (
              <tr key={r.id} className={r.estado === "anulado" ? "fila-anulada" : ""}>
                ...
                <td>
                  {r.estado === "anulado" ? (
                    <span className="chip chip-anulado">anulado</span>
                  ) : (
        ...
        {!cargando && recibos.length === 0 && (
          <div className="vacio">Sin cobranzas registradas</div>
        )}
      </div>
      {modalAbierto && (
        <ReciboModal
